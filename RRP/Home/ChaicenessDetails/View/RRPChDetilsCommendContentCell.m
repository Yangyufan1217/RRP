...
- (void)awakeFromNib {
    ...
    //修改参数
    self.nameLabel.backgroundColor = self.moneyLabel.backgroundColor = self.distanceLabel.backgroundColor = self.commentNumberLabel.backgroundColor = self.typeLabel.backgroundColor = [UIColor clearColor];
    self.nameLabel.font = self.moneyLabel.font  = self.distanceLabel.font = self.commentNumberLabel.font = self.typeLabel.font =[UIFont systemFontOfSize:12];
...
- (void)showDataWithModel:(RRPPeripheryModel *)model
{
    ...
    self.moneyLabel.text = [NSString stringWithFormat:@"￥%@起",price];
